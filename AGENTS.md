...
- **共享 thirdCategoryId 目录的侧边栏铺产品**：当某三级目录下多个产品共享同一 `thirdCategoryId`（如 `brazing-torches` 6 个焊枪、`brazing-rods` 3 个焊条），侧边栏会把该目录视为"共享容器"并遵循相对层级（与 valves 下 01/02/03/04 子目录并列一致）：父级 `welding-gas-equipment` 展开时只显示 `01 Brazing Torches`、`02 Brazing Rods` 两个并列子目录；**仅当用户点开该 third 目录（activeThirdCategory/overview/产品高亮命中该 third 家族）时**，才在其下展开 Category Overview + `01..0n` 缩进产品项（链接 `/products/<cat>/<sub>/<third>#product-<id>`）。即 `showDetails = 是共享容器 && !!expandedThirds[thirdKey]`，避免父级一展开就把所有产品铺开；展开/收起完全由 `expandedThirds` 控制（点击共享容器目录行即 toggle，chevron 在目录名内随展开旋转 90°）；URL 深链进入该 third 页时 effect 自动预置展开（对 active third 的 shared 容器 set expandedThirds=true）。产品卡 `id=product-<id>` 与 `#product-<id>` hash 滚动定位配合（Products.tsx 增补了 hash 解析 useEffect）。**共享容器三级目录下的产品详情聚焦**：点击该类目录下的某个产品名链接（`/products/<cat>/<sub>/<third>#product-<id>`）时，主内容区只渲染该单一产品（focusedProducts 派生逻辑：hash 命中且存在于当前 filteredProducts 时取单条、否则展示该目录全部产品；未带 hash 时仍展示全部），实现"点哪个型号就显示哪个"，对 Brazing Torches / Brazing Rods 等共享 thirdCategoryId 目录统一生效
