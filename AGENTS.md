...
- **共享 thirdCategoryId 目录的侧边栏铺产品**：当某三级目录下多个产品共享同一 `thirdCategoryId`（如 `brazing-torches` 6 个焊枪、`brazing-rods` 3 个焊条），侧边栏渲染会把该目录下已发布产品（`isProductPublished`）以 `01..0n` 缩进子项铺开，链接到 `/products/<category>/<subCategory>/<third>#product-<id>`；Category Overview（belongsToThird）置于产品列表之上，与 filter-driers 的"相对框架"（类目下先 overview 再产品）一致，且只在该三级目录内部生效、不会上浮到 welding-gas-equipment 一级。产品卡 `id=product-<id>` 与 `#product-<id>` hash 滚动定位配合（Products.tsx 增补了 hash 解析 useEffect）
